...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 27 14:29:01 2022
                ...
                for sub in subs:
                    for obj in objs:
                        ...
                        if is_pas:  # reverse object / subject for passive
                            svos.append([expand(obj, tokens, visited),
                                         "!" + v.lemma_ if verbNegated or objNegated else v.lemma_, expand(sub, tokens, visited)])
                            svos.append([expand(obj, tokens, visited),
                                         "!" + v2.lemma_ if verbNegated or objNegated else v2.lemma_, expand(sub, tokens, visited)])
                        else:
                            svos.append([expand(sub, tokens, visited),
                                         "!" + v.lower_ if verbNegated or objNegated else v.lemma_, expand(obj, tokens, visited)])
                            svos.append([expand(sub, tokens, visited),
                                         "!" + v2.lower_ if verbNegated or objNegated else v2.lemma_, expand(obj, tokens, visited)])
            else:
...
def data_import(path):
    ...
    for x in list_of_files:
        file=open(path + '/' + x,"r",errors='ignore')
        sec_content=file.read()
        content+=sec_content
        file.close()
    content=str(content)
    return content

# Extracting and return the triples
def extract_triple_list(path):
    text=data_import(path)
